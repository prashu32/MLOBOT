...
from mlobot.utils import admin_cmd, load_module, remove_plugin

from .. import ALIVE_NAME

DELETE_TIMEOUT = 5
thumb_image_path = "./resources/Mlobot.jpeg"
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "Mlobot User"


@mlobot.on(admin_cmd(pattern=r"send (?P<shortname>\w+)", outgoing=True))
@mlobot.on(sudo_cmd(pattern=r"send (?P<shortname>\w+)", allow_sudo=True))
async def send(event):
    ...
    the_plugin_file = "./mlobot/plugins/{}.py".format(input_str)
...
@mlobot.on(admin_cmd(pattern="install"))
        ...
        try:
            downloaded_file_name = (
                await event.client.download_media(  # pylint:disable=E0602
                    ...
                    "mlobot/plugins/",  # pylint:disable=E0602
                )
            )
            if "(" not in downloaded_file_name:
                ...
                await event.edit(
                    "MloBot Succesfully Installed The Plugin ab naacho bae `{}`".format(
                ...
                await event.edit(
                    "**Error!**\nPlugin cannot be installed!\nMight have been pre-installed ab kitni baar Kroge boss."
                )
...
@mlobot.on(admin_cmd(pattern=r"unload (?P<shortname>\w+)$"))
async def unload(event):
    ...
    try:
        ...
        await event.edit(f"MloBot has successfully unloaded {shortname}")
    except Exception as e:
        await event.edit(
            "MloBot has successfully unloaded {shortname}\n{}".format(
                shortname, str(e)
            )
        )


@mlobot.on(admin_cmd(pattern=r"load (?P<shortname>\w+)$"))
async def load(event):
    ...
    try:
        ...
        await event.edit(f"Mlobot has successfully loaded Saar {shortname}")
    except Exception as e:
        await event.edit(
            f"MloBot could not load {shortname} because of the following error.\n{str(e)}"
        )


@mlobot.on(admin_cmd(pattern=r"installall$"))
async def install(event):
    if event.fwd_from:
        return
    ...
    for ixo in total_doxx:
        ...
        downloaded_file_name = await event.client.download_media(
            await event.client.get_messages(event.chat_id, ids=mxo), "mlobot/plugins/"
        )
        if "(" not in downloaded_file_name:
            path1 = Path(downloaded_file_name)
            shortname = path1.stem
