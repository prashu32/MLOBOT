#    MLOBOT - UserBot
#    CRDIT GOES TO TELEBOT TAKEN FOR MLOBOT
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        if previous_message.media:
            downloaded_file_name = await mlobot.download_media(
    ...
    try:
        ...
        if FBAN == "@MBBS_LOVER" or FBAN == "@MBBS_LOVER":
            await event.edit("Something went wrong.")
            return
    ...
    if not len(fedList):
        for a in range(3):
            async with mlobot.conversation("@MissRose_bot") as bot_conv:
    ...
    try:
        await mlobot.send_message(chat, f"/start")
    except BaseException:
    ...
    for fed in fedList:
        if Config.EXCLUDE_FED and fed in excludeFed:
            ...
            exCount += 1
            continue
        await mlobot.send_message(chat, f"/joinfed {fed}")
        await asyncio.sleep(3)
        await mlobot.send_message(chat, f"/fban {FBAN} {REASON}")
        await asyncio.sleep(3)
    await event.edit(
        f"SuperFBan Completed my son. Affected {len(fedList) - exCount} feds.\n#TB"
    )
...
@mlobot.on(admin_cmd("superunfban ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
