...
@mlobot.on(admin_cmd(pattern=r"gmute ?(\d+)?"))
async def startgmute(event):
    private = False
    if event.fwd_from:
        return
    elif event.is_private:
        await event.edit("Putting Dick🍆 💦 In Son mouth bsdk!!")
...
@mlobot.on(admin_cmd(pattern=r"ungmute ?(\d+)?"))
async def endgmute(event):
    ...
    if event.fwd_from:
        ...
        await event.edit(
            "Taking Out Dick from Son mouth relax....\n\n       Today Sex Done😁 "
        )
        await asyncio.sleep(3)
        private = True
    reply = await event.get_reply_message()
    ...
    try:
        ...
        await event.edit("Son Feeling Good..... Now speak bsdk beta🍆🍆")
